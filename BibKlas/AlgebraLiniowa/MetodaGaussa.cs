...
//Podać referencje do Numerics bo biblioteka ta zawiera klasę Complex
namespace BibKlas.AlgebraLiniowa {
    public static class MetodaGaussa {
        ...
        public static string[] Komunikat = { 
            ...
            /*3 */ "Warunek przerwania poszukiwania elementu głównego" + " metody eliminacji Gaussa w metodzie RozRowMacGaussa "
         };
       ...
       double[] X, double eps) {
            ...
            if (N == M && N == R) {
                ...
                for (i = 1; i <= N; i++) {
                    ...
                    if (T < eps) { //Warunek przerwania poszukiwania elementu głównego
                    ...
                    if (i == k) { ZT = A[i, i]; }
                        ...
                        ZT = A[k, i];
                        for (j = N; j >= i; j--) { //Zamiana wiersza k-tego z i-tym macierzy A
        ...
        /// <returns>return 0 brak błędu,
        /// inna wartość = numer błędu</returns>
        public static int RozRowMacGaussa(Complex[,] A, Complex[] B,
            ...
            if (N == M && N == R) {
                ...
                for (i = 1; i <= N; i++) {
                    ...
                    if (i == k) { ZT = A[i, i]; }
                        ...
                        ZS = B[k]; B[k] = B[i]; B[i] = ZS;
                        ZT = A[k, i];
                        for (j = N; j >= i; j--) { //Zamiana wiersza k-tego z i-tym macierzy MacA
                            ZS = A[k, j]; A[k, j] = A[i, j]; A[i, j] = ZS;
                        }//j
                    }
                    ZT = a1 / ZT;
